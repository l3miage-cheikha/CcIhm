...
export class CartoService {
  ...
  private readonly _colors = signal<readonly string[]>(
    (this.data()?.region.departements ?? []).map( () => '#0000FF' )
  );
  public readonly colors = this._colors.asReadonly();
  public readonly couleurCommune = computed(() => {
    const data = this._data();
    const colors = this._colors();

    if (!data?.commune || !data?.region) return "#000000"; // couleur par défaut

    const codeDepCommune = data.commune.data.code
    const index = data.region.departements.findIndex(dep => dep.data.code === codeDepCommune);

    return index >= 0 ? colors[index] : "#000000";
  });
